...
# indicators
user_indicator = Label(root, font=400, text="USER", bg="#9b59b6", fg="white")
comp_indicator = Label(root, font=400, text="COMPUTER",
                       bg="#9b59b6", fg="white")
...
def checkWin(player, computer):
    if player == computer:
        updateMessage("Its a tie!!!\U0001F610")
    elif player == "rock":
        if computer == "paper":
            updateMessage("You lost\U0001F914")
            updateCompScore()
        else:
            updateMessage("You Won\U0001F602")
            updateUserScore()
    elif player == "paper":
        if computer == "scissor":
            updateMessage("You lost\U0001F914")
